...
class EventSearchEngine:
    ...
    async def search_all_sources(self, lat: float, lng: float, radius_km: int = 4) -> List[Dict[str, Any]]:
        """
    ...
    async def _search_popular_places(self, lat: float, lng: float, radius_km: int) -> List[Dict[str, Any]]:
        ...
        # Популярные места в Москве (пример)
        popular_places = [
            {
                "name": "Парк Горького",
                "lat": 55.7298,
                "lng": 37.6008,
                "events": [
                    {"title": "Вечерние концерты в парке", "time": "19:00", "description": "Живая музыка на открытом воздухе"}
                ]
            },
            {
                ...
                "lng": 37.6208,
                "events": [
                    {"title": "Выставка современного искусства", "time": "10:00", "description": "Новые работы современных художников"}
                ]
            },
            {
                "name": "Большой театр",
                ...
                "lng": 37.6186,
                "events": [
                    {"title": "Балет 'Лебединое озеро'", "time": "19:00", "description": "Классический балет"}
                ]
            }
        ]
...
async def enhanced_search_events(lat: float, lng: float, radius_km: int = 4) -> List[Dict[str, Any]]:
    """
