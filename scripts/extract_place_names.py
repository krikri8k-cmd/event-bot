#!/usr/bin/env python3
"""
Скрипт для извлечения названий мест из Google Maps ссылок
и добавления их в файл перед ссылками
"""
...
async def get_place_name(url: str) -> str:
    ...
    try:
        ...
        print(f"⚠️ Ошибка при получении названия для {url}: {e}")
    return None


async def process_file(file_path: str) -> None:
    """Обрабатывает файл, добавляя названия перед ссылками"""
    ...
    while i < len(lines):
        ...
        if re.match(r"https?://(maps\.app\.goo\.gl|goo\.gl/maps|maps\.google\.com|www\.google\.com/maps)", line):
            ...
            if not has_name_before:
                ...
                print(f"⏭️ Пропускаю ссылку (уже есть название): {line[:50]}...")
...
async def main():
    if len(sys.argv) < 2:
        print("Использование: python scripts/extract_place_names.py <путь_к_файлу>")
        print("Пример: python scripts/extract_place_names.py interesting_places_example.txt")
        sys.exit(1)
