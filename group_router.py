...
@group_router.message(lambda message: message.text == "Добавить в чат")
async def handle_add_to_chat_button(message: Message, bot: Bot, session: AsyncSession):
    """Обработчик кнопки 'Добавить в чат' - вызывает команду /start"""
    logger.info(f"🔘 Кнопка 'Добавить в чат' от пользователя {message.from_user.id} в чате {message.chat.id}")
    ...
    if message.chat.type in ("group", "supergroup"):
        ...
        try:
            ...
            start_keyboard = ReplyKeyboardMarkup(
                keyboard=[[KeyboardButton(text="Добавить в чат")]],
