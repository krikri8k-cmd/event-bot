...
async def handle_bot_added_to_group(message: Message, bot: Bot):
    ...
    admin_text = (
        ...
        "📋 **ВАЖНО! Порядок настройки:**\n"
        "1️⃣ **Сначала включить «Управление группой»**\n"
        "2️⃣ **Затем включить «Удаление сообщений»**\n"
        "3️⃣ **Затем включить «Управление историями»**\n"
        "4️⃣ **«Закрепление сообщений»** (уже включено)\n\n"
        ...
        "📱 **Как настроить (НОВЫЙ СПОСОБ):**\n"
        "1. **Сначала добавьте бота** как обычного участника\n"
        "2. Нажмите на название группы\n"
        "3. Перейдите в «Администраторы»\n"
        "4. Нажмите «Добавить администратора»\n"
        "5. Найдите EventAroundBot и назначьте админом\n"
        "6. **Строго следуйте порядку выше** - без «Управления группой» не получится включить остальные!\n\n"
        "✅ **Преимущество:** Теперь можно изменять разрешения в любое время через админ-панель!"
    )
