...
def start_button_kb() -> ReplyKeyboardMarkup:
    """Создаёт клавиатуру только с кнопкой START для новых пользователей"""
    keyboard = [[KeyboardButton(text="START")]]
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True)


def is_new_user(user_id: int) -> bool:
    """Проверяет, является ли пользователь новым (создан менее 1 минуты назад)"""
    try:
        with get_session() as session:
            user = session.get(User, user_id)
            if not user:
                return True  # Если пользователя нет в БД, считаем новым

            # Проверяем, создан ли пользователь менее 1 минуты назад
            from datetime import UTC, datetime, timedelta

            now = datetime.now(UTC)
            one_minute_ago = now - timedelta(minutes=1)

            return user.created_at_utc > one_minute_ago
    except Exception as e:
        logger.warning(f"Ошибка проверки нового пользователя {user_id}: {e}")
        return False  # В случае ошибки считаем не новым
...
async def cmd_start(message: types.Message, state: FSMContext, command: CommandObject = None):
    ...
    logger.info(f"cmd_start: пользователь {user_id}, новый: {is_new}")

    # Разная логика для личных и групповых чатов
    if chat_type == "private":
        if is_new:
            # Новый пользователь - показываем только кнопку START
            welcome_text = (
                "Привет! 🌟 Я EventAroundBot - твой проводник по активностям рядом!\n\n"
                "📍 Что рядом: находи события и встречи в радиусе 5–20 км\n"
                "🎯 Квесты на районе: автоматизированный подбор заданий и мест с наградами 🚀\n\n"
                "➕ Создавай свои события: организуй встречи и приглашай друзей\n"
                "🔗 Поделиться: добавь бота в чат — и у вас появится своя лента встреч и планов, видимая только для своих внутри сообщества\n\n"
                "🚀 Нажми START чтобы начать приключение!"
            )
            await message.answer(welcome_text, reply_markup=start_button_kb())
        else:
            # Существующий пользователь - показываем полное меню
            welcome_text = (
                "Привет! 🌟 Я EventAroundBot - твой проводник по активностям рядом!\n\n"
                "📍 Что рядом: находи события и встречи в радиусе 5–20 км\n"
                "🎯 Квесты на районе: автоматизированный подбор заданий и мест с наградами 🚀\n\n"
...
async def on_my_events(message: types.Message):
    ...
    if not events and not all_participations:
        text_parts = [
            ...
            "💡 **Что можно сделать:**\n",
            "• Создать событие через '➕ Создать'\n",
            "• Найти интересные события через '📍 Что рядом'",
        ]
...
async def on_my_tasks(message: types.Message):
    ...
    message_text += (
        "⏰ Для мотивации даем 24 часа на выполнение, после задание сгорает и ты всегда можешь взять новое\n\n"
    )
