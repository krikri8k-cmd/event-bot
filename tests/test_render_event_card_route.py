...
class TestRenderEventCardRoute:
    ...
    def test_telegram_private_shows_description_and_author(self):
        event = self.base_event(
            source="telegram",
            type="source",
            title="Dancing Until Dawn",
            description="Мега вечеринка с DJ set на открытом воздухе.",
            venue_name="Savaya Bali",
            lat=-8.84,
            lng=115.14,
            organizer_id=456,
            organizer_username="Fincontro",
            community_link="",
            url="https://t.me/c/5179811176/286232",
        )
        html = render_event_html(event, 3)
        assert "📝 Мега вечеринка с DJ set на открытом воздухе." in html
        assert "@Fincontro" in html
        assert "Подробнее" not in html
        assert "🚗" not in html

    def test_telegram_public_shows_post_link(self):
        event = self.base_event(
            source="telegram",
            type="source",
            venue_name="Savaya Bali",
            community_link="https://t.me/publicgroup",
            url="https://t.me/publicgroup/99",
        )
        html = render_event_html(event, 1)
        assert "🔗" in html
        assert "Подробнее" in html
